												...
												<td align="right">
													<c:if test="${!menu.leaf}">
														<c:if test="${menu.expanded}">
															<input type="checkbox" ${menu.checked} id="${menu.parent.id}_${menu.id}" value="${menu.id}" name="menus" onclick="menu.cascadeCheck(true, this, true);"/>
															<a href="${pageContext.request.contextPath}/role-menu/${menu.id }/expand?role.id=${id}">
																<img border="0" alt="折 叠" src="${pageContext.request.contextPath}/images/minus.jpg">
															</a>
														</c:if>
														<c:if test="${!menu.expanded}">
															<input type="checkbox" ${menu.checked} id="${menu.parent.id}_${menu.id}" value="${menu.id}" name="menus" onclick="menu.cascadeCheck(true, this, true);"/>
															<a href="${pageContext.request.contextPath}/role-menu/${menu.id}/expand?role.id=${id}">
																<img border="0" alt="展 开" src="${pageContext.request.contextPath}/images/plus.jpg">
															</a>
														</c:if>
													</c:if>
													<c:if test="${menu.leaf}">
